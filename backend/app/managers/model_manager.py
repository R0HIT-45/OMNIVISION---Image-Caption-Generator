...
from app.utils.memory_utils import clear_gpu_memory
from app.models.registry import get_model_class
...
class ModelManager:
    ...
    def _get_model_id_for_key(self, model_key: str) -> str:
        if model_key == "blip": return settings.BLIP_MODEL
        if model_key == "clip": return settings.CLIP_MODEL
        if model_key == "translation": return settings.TRANSLATION_MODEL
        if model_key == "tts": return settings.TTS_MODEL
        raise ValueError(f"Unknown model key: {model_key}")

    def _get_category_for_key(self, model_key: str) -> str:
        if model_key == "blip": return "caption"
        if model_key == "clip": return "embedding"
        if model_key == "translation": return "translation"
        if model_key == "tts": return "tts"
        raise ValueError(f"Unknown model key: {model_key}")
    ...
    def get_model(self, model_key: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if state in [ModelState.LOADING, ModelState.UNLOADING]:
                raise RuntimeError(f"Race condition detected: Model {model_key} is currently {state.value}")
